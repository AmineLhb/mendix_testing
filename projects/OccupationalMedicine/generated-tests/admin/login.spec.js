...
test.describe('Login (admin)', { tag: '@smoke' }, () => {
  ...
  test('logs in successfully with valid credentials', async ({ page }) => {
    ...
    let status, body;
    await test.step('When they submit valid admin credentials', async () => {
      await basePage.mx('textBox1').fill(process.env.MENDIX_TEST_USERNAME_ADMIN ?? '');
      await basePage.mx('textBox2').fill(process.env.MENDIX_TEST_PASSWORD_ADMIN ?? '');
    ...
    await test.step('Then they log in as the Administrator role and land on the admin console home page', async () => {
      basePage.expectApiSuccess({ status, body });
      expect(body.roles).toContain('Administrator');
      // Verified 2026-09-07 (explorer/discover-roles.js): unlike medecin/infirmier,
      // this account's Name attribute matches the login username exactly ("MxAdmin"),
      // so an exact comparison is fine here.
      expect(body.user.attributes.Name.value).toBe(process.env.MENDIX_TEST_USERNAME_ADMIN);

      await basePage.waitForMendixIdle();
      await basePage.expectNoErrorMessage();

      // Verified 2026-09-07: the admin role lands on a completely different
      // internal admin console ("Page d'accueil") rather than the visit-list
      // page other roles land on — actionButton14 doesn't exist here, so use
      // the console's own page title as the past-login signal instead.
