...
test.describe('Login (medecin)', { tag: '@smoke' }, () => {
  ...
  test('logs in successfully with valid credentials', async ({ page }) => {
    ...
    let status, body;
    await test.step('When they submit valid medecin credentials', async () => {
      await basePage.mx('textBox1').fill(process.env.MENDIX_TEST_USERNAME_MEDECIN ?? '');
      await basePage.mx('textBox2').fill(process.env.MENDIX_TEST_PASSWORD_MEDECIN ?? '');
    ...
    await test.step('Then they log in as the Medecin_Travail role and land on the visit list page', async () => {
      basePage.expectApiSuccess({ status, body });
      expect(body.roles).toContain('Medecin_Travail');
      // Verified 2026-09-07 (explorer/discover-roles.js): this account's Name
      // attribute is stored uppercase ("MEDECIN") regardless of the login
      // form's case, so compare case-insensitively rather than exact-match.
      expect(body.user.attributes.Name.value.toUpperCase()).toBe(
        (process.env.MENDIX_TEST_USERNAME_MEDECIN ?? '').toUpperCase()
      );

      await basePage.waitForMendixIdle();
      await basePage.expectNoErrorMessage();

      // Verified 2026-09-07: medecin lands on the same "Ajouter une visite"
